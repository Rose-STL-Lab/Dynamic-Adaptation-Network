...
for i in range(1,26):
    ...
    torch.save(torch.from_numpy(Ws[1:]).double().float(), rawdata_direc + "data" + str(int(buoyancy_factor))+".pt")
...
for i in range(1,26):
    ...
    for j in range(0, len(data) - 50):
        torch.save(data[j:j+50].transpose(-1,-2).transpose(-2,-3).double().float(), preprocessed_direc + "data" + str(i) + "/" + "sample_" + str(j) + ".pt")
